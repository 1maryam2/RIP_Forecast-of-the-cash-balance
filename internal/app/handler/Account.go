...
type AsyncRequest struct {
	ID         uint        `json:"id"`
	InitialSum float64     `json:"initial_sum"` // Это "Он"
	Items      []AsyncItem `json:"items"`       // Из этого Python посчитает "П" и "Р"
}
...
func (h *Handler) RegisterRoutes(router *gin.Engine, jwtService *auth.JWTService, redisClient *redis.Client) {
	...
	api.GET("/cash-forecasts/cart", h.GetCartIcon)
...
func (h *Handler) GetAccounts(c *gin.Context) {
	...
	// Если это обычный запрос - просто отдаем данные
	if mode != "long_polling" {
		h.returnAccountsList(c)
		return
	}

	// --- ЛОГИКА LONG POLLING ---

	// 1. Подписываемся на обновления
	updateChan := h.EventManager.Subscribe()
	defer h.EventManager.Unsubscribe(updateChan)

	// 2. Ждем события ИЛИ таймаута (например, 30 секунд)
	select {
	case <-updateChan:
		// Пришел сигнал: данные изменились!
		h.returnAccountsList(c)
	case <-time.After(30 * time.Second):
		// Таймаут: изменений не было, возвращаем пустой ответ или 304
		// Клиент должен тут же перезапросить
		h.successResponse(c, gin.H{"timeout": true, "data": []ds.Account{}})
	}
}

// Вспомогательная функция, чтобы не дублировать код
func (h *Handler) returnAccountsList(c *gin.Context) {
...
// @Description Получение ID заявки-черновика и количества услуг в ней. Для неавторизованных пользователей возвращает {0, -1}.
// @Tags cash-forecasts
// @Security BearerAuth
// @Produce json
...
func (h *Handler) FormCashForecast(c *gin.Context) {
	...
	// 1. Получаем заявку с вложенными счетами (Account)
	// В репозитории в методе GetCashForecastByID должен быть .Preload("FundsApplicationItem.Account")
	...
	// 2. Валидация (проверяем, заполнены ли поля, есть ли счета)
	// Используем твой существующий метод, он ставит статус Formed и проверяет поля
	if err := h.Repository.FormCashForecast(uint(appID)); err != nil {
		h.errorResponse(c, http.StatusBadRequest, err.Error())
		return
	}

	// 3. Ставим статус "processing", чтобы фронтенд показал спиннер/часики
	// (Убедись, что метод SetStatus добавлен в репозиторий, как обсуждали выше)
	if err := h.Repository.SetStatus(uint(appID), ds.Processing); err != nil {
		h.errorResponse(c, http.StatusInternalServerError, "Failed to set processing status")
		return
	}

	// 4. Собираем данные для Python (Он, П, Р)
	var itemsForAsync []AsyncItem
	for _, item := range app.FundsApplicationItem {
		// item.Account.Type должен содержать "income" или "expense"
		itemsForAsync = append(itemsForAsync, AsyncItem{
			Amount: item.Amount,
			Type:   item.Account.Type,
		})
	}

	// 5. Отправляем в Python АСИНХРОННО (в отдельной горутине)
	go func(applicationID uint, initialSum float64, items []AsyncItem) {
		reqData := AsyncRequest{
			ID:         applicationID,
			InitialSum: initialSum, // Это "Он"
			Items:      items,      // Список для расчета "П" и "Р"
		}
		...
		// URL твоего Python сервиса (calculator/views.py -> calculate_forecast)
		resp, err := http.Post("http://localhost:8000/calculate/", "application/json", bytes.NewBuffer(jsonData))
		if err != nil {
			fmt.Printf("Error sending to async service: %v\n", err)
			return
		}
		defer resp.Body.Close()

		fmt.Printf("Sent to Python. Response status: %s\n", resp.Status)
	}(uint(appID), app.InitialSum, itemsForAsync)

	// 6. Сразу отвечаем пользователю, не дожидаясь расчета
	h.successResponse(c, gin.H{"message": "Application is being calculated..."})
}

func (h *Handler) ReceiveCalculationResult(c *gin.Context) {
	...
	err := h.Repository.UpdateApplicationResult(resp.ID, resp.Result)
	if err != nil {
		h.errorResponse(c, http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *Handler) CompleteCashForecast(c *gin.Context) {
	...
	completedApp, err := h.Repository.CompleteCashForecast(uint(appID), moderatorID)
	if err != nil {
		h.errorResponse(c, http.StatusBadRequest, err.Error())
		return
	}
	h.successResponse(c, completedApp)
}
...
func (h *Handler) AddAccountToCashForecast(c *gin.Context) {
	...
	h.successResponse(c, gin.H{"application_id": draftApp.ID})
}
