...
const HardcodedToken = "super-secret-key-2025"
...
type RevenuePredictionRequest struct {
	ID                uint          `json:"id"`
	InitialSum        float64       `json:"initial_sum"`
	Quarter           int           `json:"quarter"`
	VerificationToken string        `json:"verification_token"`
	Items             []RevenueItem `json:"items"`
}

type RevenuePredictionResult struct {
	ID                uint    `json:"id"`
...
func (h *Handler) FormCashForecast(c *gin.Context) {
	...
	var itemsForAsync []RevenueItem
	for _, item := range app.FundsApplicationItem {
		itemsForAsync = append(itemsForAsync, RevenueItem{
			Amount: item.Amount,
			Type:   item.Account.Type,
		})
	}
	go func(applicationID uint, initialSum float64, quarter int, items []RevenueItem) {
		reqData := RevenuePredictionRequest{
			ID:                applicationID,
			InitialSum:        initialSum,
			Quarter:           quarter,
			VerificationToken: HardcodedToken,
			Items:             items,
		}
		...
		resp, err := http.Post("http://localhost:8000/api/predict-revenue", "application/json", bytes.NewBuffer(jsonData))
		if err != nil {
	...
	}(uint(appID), app.InitialSum, app.Quarter, itemsForAsync)

	h.successResponse(c, gin.H{"message": "Revenue prediction started..."})
}
func (h *Handler) UpdateForecastResult(c *gin.Context) {
	var resp RevenuePredictionResult
	if err := c.ShouldBindJSON(&resp); err != nil {
		h.errorResponse(c, http.StatusBadRequest, err.Error())
		return
	}
	if resp.VerificationToken != HardcodedToken {
		h.errorResponse(c, http.StatusForbidden, "Invalid verification token")
		return
	}
