...
func (r *Repository) GetCashForecastsList(filter ds.ApplicationFilter) ([]ds.FundsApplication, error) {
	...
	dbQuery := r.db
	dbQuery = dbQuery.Where("status NOT IN (?, ?)", "deleted", "draft")
...
func (r *Repository) CompleteCashForecast(appID uint, moderatorID uint) (*ds.FundsApplication, error) {
	var app ds.FundsApplication
	if err := r.db.First(&app, appID).Error; err != nil {
		return nil, err
	}
	if app.Status != ds.Formed {
		return nil, errors.New("only formed application can be completed")
	}
	...
	if err := r.db.Model(&ds.FundsApplication{}).Where("id = ?", appID).Updates(updates).Error; err != nil {
		return nil, err
	}
	app.Status = ds.Completed
	app.ModeratorID = &moderatorID
	app.CompletedAt = &now
	app.Result = calculatedResult
	return &app, nil
}
...
// Установить статус
func (r *Repository) SetStatus(appID uint, status ds.FundsApplicationStatus) error {
	return r.db.Model(&ds.FundsApplication{}).Where("id = ?", appID).Update("status", status).Error
}

// Сохранить результат от питона
func (r *Repository) UpdateApplicationResult(appID uint, result float64) error {
	now := time.Now()
	updates := map[string]interface{}{
		"status":       ds.Completed, // Заявка готова!
		"result":       result,
		"completed_at": &now,
	}
	return r.db.Model(&ds.FundsApplication{}).Where("id = ?", appID).Updates(updates).Error
